...
public class SampledNodeDateRandomWalkerSB3 extends TreeOperator {
    ...
    public void initAndValidate() {
		...
		//  Detect genes
		for (BEASTInterface output : treeInput.get().getOutputs()) {
			if (output instanceof GeneTreeForSpeciesTreeDistribution) {
				GeneTreeForSpeciesTreeDistribution gene = (GeneTreeForSpeciesTreeDistribution)output;
				if (!geneTreesInput.get().contains(gene)) {
					geneTreesInput.get().add(gene);
				}
			}
		}
    ...
    public double proposal() {
        ...
        for (GeneTreeForSpeciesTreeDistribution gene : geneTreesInput.get()) {
        	...
        	// Double check for negative branch lengths - this will only happen if there are multiple nodes mapped to the branch
        	for (Node n : geneTree.getNodesAsArray()) {
        		if (n.getLength() < 0) {
        			//Log.warning("Unexpected tip date adjustment: negative branch length");
        			return Double.NEGATIVE_INFINITY;
        		}
        	}
