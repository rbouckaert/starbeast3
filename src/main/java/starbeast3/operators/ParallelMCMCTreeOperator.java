...
import beast.base.spec.inference.operator.Transform.LogConstrainedSumTransform;
...
//				}
				...
				for (StateNode s : stateNodeList) {
					...
					// No transform
						...
						RealRandomWalkOperator op = new RealRandomWalkOperator();
						op.initByName("parameter", rp, "scaleFactor", 0.5, "weight", 0.5);
						operators.add(op);
	...
	public List<StateNode> getTabooStateNodes(List<List<ParallelMCMCTreeOperatorTreeDistribution>> dists, State mainState){
		...
		for (int i = 0; i < mainState.getNrOfStateNodes(); i ++) {
			...
			if (state instanceof RealVectorParam<?>) {
