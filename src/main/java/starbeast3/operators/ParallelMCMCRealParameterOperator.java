		...
		for (Distribution d : distributions) {
			...
			if (stateNodeSet.size() > 0) {
				...
				for (StateNode s : stateNodeSet) {
					...
					if (rp instanceof SimplexParam) {
						
						DeltaExchangeOperator op = new DeltaExchangeOperator();
						op.initByName("parameter", rp, "delta", 0.2, "weight", 2.0);
						operators.add(op);
						f = new Transform.LogConstrainedSumTransform(rp, 1.0);
						
					} else {
						
	
						ScaleOperator op = new ScaleOperator();
						op.initByName("parameter", rp, "scaleFactor", 0.5, "weight", 1.0);
						operators.add(op);
						f = new Transform.LogTransform(rp);
					}
