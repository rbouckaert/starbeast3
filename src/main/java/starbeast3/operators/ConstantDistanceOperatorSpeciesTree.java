...
public class ConstantDistanceOperatorSpeciesTree extends TreeOperator {
	...
	final public Input<RealVectorParam<NonNegativeReal>> popSizeInput = new Input<>("popsizes", "the constant population sizes associated with nodes in the tree.");
    ...
    private RealVectorParam<NonNegativeReal> popsizes;
    ...
    List<GeneTreeForSpeciesTreeDistribution> geneTreeDistributions;
    ...
    public double proposal() {
       ...
       if (clockModel != null) {
	       ...
	       switch(clockModel.getRateMode()) {
		       ...
		       case rates: {
		    	   if (nodeNr >= rates.size()) return Double.NEGATIVE_INFINITY;
		    	   r_x = rates.get(nodeNr);
		    	   r_L = rates.get(leftNr); // Rate of branch above left child
		    	   r_R = rates.get(rightNr); // Rate of branch above right child
		    	   break;
		       }
       ...
       if (clockModel != null) {
	       ...
	       switch(clockModel.getRateMode()) {
		       ...
		       case rates: {
		           ...
		           rates.set(leftNr, r_L_);
		           rates.set(rightNr, r_R_);
		           logJD_quantiles = 0;
		    	   break;
		       }
       ...
       if (proposeNewPopulationSizes) {
	       ...
	       popsizes.set(nodeNr, N_x_);
	       popsizes.set(leftNr, N_L_);
	       popsizes.set(rightNr, N_R_);
