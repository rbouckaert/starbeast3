...
public class PopSizeGibbsSampler extends Operator {
	...
	final public Input<RealVectorParam<NonNegativeReal>> popSizesInput = new Input<>("popSizes", "constant population size parameter, one dimension for each branch of the species tree", Validate.REQUIRED);
	final public Input<InverseGamma> priorInput = new Input<>("gammaprior", "gamma distributed prior for population sizes", Validate.REQUIRED);
	...
	RealVectorParam<NonNegativeReal> popSizes;
	RealScalar<PositiveReal> priorAlpha, priorBeta;
	InverseGamma prior;
	...
	public void initAndValidate() {
		...
		if (geneTreeDistributions != null && geneTreeDistributions.size() > 0) {
			...
			if (speciesTree.getNodeCount() != popSizes.size()) {
				throw new IllegalArgumentException("The dimension of the population size parameter (" + popSizes.size()+ ") "
	...
	public double proposal() {
		...
		double[] newPopSizes = new double[popSizes.size()];
