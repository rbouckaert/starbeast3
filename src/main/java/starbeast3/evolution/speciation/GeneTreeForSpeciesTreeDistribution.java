...
public class GeneTreeForSpeciesTreeDistribution extends Distribution {
    ...
    public double calculateLogP() {
        ...
        logP = popModel.getRootBranchLogP(treeInput.get().getRoot().getHeight(), popSizesBottom.get(speciesNodeWithGeneTreeRoot), 
        ...
        for (int speciesNodeI = 0; speciesNodeI < speciesNodeCount; speciesNodeI++) {
            ...
            if (isDirtyBranch(speciesNodeI) || popModel.isDirtyBranch(speciesNode)) {
                ...
                perBranchLogP[speciesNodeI] = popModel.calculateBranchLogP(lineagesBottom, ploidy, popSizesBottom.get(speciesNodeI), branchCoalescentTimes, k); 
            }
    ...
    public void sample(final State state, final Random random) {
        ...
        RealVectorParam<NonNegativeReal> popSizes = speciesTreePrior.getPopulationSizes();
    ...
    private List<Node> sampleBranchRecursive(final Node node, final Random random, final RealVectorParam<NonNegativeReal> popSizes) {
    	
    	
    	List<Node> geneTreeNodesInLineage = new ArrayList<Node>();
    	
    	// Population size of this species branch
    	final double popSizeBranch = popSizes.get(node.getNr()) * ploidy;
